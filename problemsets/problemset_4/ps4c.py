...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        ...
        words_in_message = decrypted_message.get_text().split()
        valid_count = 0
        for word in words_in_message:
            if is_word(word_list, word):
                valid_count += 1
        if valid_count >= num_of_valid_words:
            num_of_valid_words = valid_count
            res = ps4b.PlaintextMessage(decrypted_message.get_text(), pad)
    return res
...
def decode_story():
    ...
    pads = get_story_pads()
    res = decrypt_message_try_pads(story, pads)
    return res.get_text()
