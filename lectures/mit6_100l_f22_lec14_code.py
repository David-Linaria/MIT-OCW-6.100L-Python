...
def find_grades(grades, students):
    ...
    for stu in students:
        try:
            grades_of_students.append(grades[stu])
        except:
            print('Does not find this student.')
            grades_of_students.append(' ')
...
def is_inverse(d1, d2):
    ...
    if len(d1) != len(d2):
        return False
    elif list(d1.keys()) == list(d2.values()) and list(d2.keys()) == list(d1.values()):
        return True
    else:
        return False
    #pass
